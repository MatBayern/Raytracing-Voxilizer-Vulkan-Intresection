...
void HelloVulkan::updateDescriptorSet()
{
    ...
    VkDescriptorBufferInfo dbiAabb{m_AabbBuffer.buffer, 0, VK_WHOLE_SIZE};
    writes.emplace_back(m_descSetLayoutBind.makeWrite(m_descSet, eImplicit, &dbiAabb));
...
void HelloVulkan::destroyResources()
{
    ...
    m_alloc.destroy(m_AabbBuffer);
    m_alloc.destroy(m_VoxelMatColorBuffer);
    m_alloc.destroy(m_VoxelMatIndexBuffer);
...
//
auto HelloVulkan::AABBToVkGeometryKHR()
{
    VkDeviceAddress dataAddress = nvvk::getBufferDeviceAddress(m_device, m_AabbBuffer.buffer);
...
void HelloVulkan::createAABB(const std::string& path, float voxleSize)
{
    ...
    // Debug information
    m_debug.setObjectName(m_AabbBuffer.buffer, "VoxelAabb");
    m_debug.setObjectName(m_VoxelMatColorBuffer.buffer, "VoxelMat");
    m_debug.setObjectName(m_VoxelMatIndexBuffer.buffer, "VoxelMatIdx");
